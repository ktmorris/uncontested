...
means_prematch <- wi %>% 
  ...
  summarize_at(vars(gender, pred.whi, pred.bla, pred.his, median_income, some_college, median_age), funs(mean(.)))

means_postmatch <- matches %>% 
  group_by(treat) %>% 
  summarize_at(vars(gender, pred.whi, pred.bla, pred.his, median_income, some_college, median_age), funs(sum(weight * .) / sum(weight)))

rm(matches, matches1, matches2)

qqs_post <- lapply(c("gender", "pred.whi", "pred.bla", "pred.his", "median_income", "some_college", "median_age"), function(var){
  j <- select(wi, var)
  colnames(j) <- c("t")
...
qqs_pre <- lapply(c("gender", "pred.whi", "pred.bla", "pred.his", "median_income", "some_college", "median_age"), function(var){
...
i = 1
for(var in c("gender", "pred.whi", "pred.bla", "pred.his", "median_income", "some_college", "median_age")){
...
varnames <- c("gender", "pred.whi", "pred.bla", "pred.his", "median_income", "some_college", "median_age")
