...
means_prematch <- ga %>% 
  ...
  summarize_at(vars(gender, dem, rep, yob, white, black, latino.x, median_income, some_college), funs(mean(.)))

means_postmatch <- matches %>% 
  group_by(treat) %>% 
  summarize_at(vars(gender, dem, rep, yob, white, black, latino.x, median_income, some_college), funs(sum(weight * .) / sum(weight)))

rm(matches, matches1, matches2)

qqs_post <- lapply(c("gender", "dem", "rep", "yob", "white", "black", "latino.x", "median_income", "some_college"), function(var){
  j <- select(ga, var)
  colnames(j) <- c("t")
...
qqs_pre <- lapply(c("gender", "dem", "rep", "yob", "white", "black", "latino.x", "median_income", "some_college"), function(var){
...
i = 1
for(var in c("gender", "dem", "rep", "yob", "white", "black", "latino.x", "median_income", "some_college")){
...
varnames <- c("gender", "dem", "rep", "yob", "white", "black", "latino.x", "median_income", "some_college")
...
saveRDS(df, "./temp/match_table_ga.rds")
