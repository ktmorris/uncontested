...
matches <- left_join(matches, ma, by = "id")

##########
means_prematch <- ma %>% 
  group_by(uncontested) %>% 
  summarize_at(vars(gender, dem, rep, yob, pred.whi, pred.bla, pred.his, median_income, some_college), funs(mean(.)))

means_postmatch <- matches %>% 
  group_by(treat) %>% 
  summarize_at(vars(gender, dem, rep, yob, pred.whi, pred.bla, pred.his, median_income, some_college), funs(sum(weight * .) / sum(weight)))

rm(matches, matches1, matches2)

qqs_post <- lapply(c("gender", "dem", "rep", "yob", "pred.whi", "pred.bla", "pred.his", "median_income", "some_college"), function(var){
  j <- select(ma, var)
  colnames(j) <- c("t")
...
qqs_pre <- lapply(c("gender", "dem", "rep", "yob", "pred.whi", "pred.bla", "pred.his", "median_income", "some_college"), function(var){
  j <- select(ma, var, uncontested)
...
i = 1
for(var in c("gender", "dem", "rep", "yob", "pred.whi", "pred.bla", "pred.his", "median_income", "some_college")){
...
varnames <- c("gender", "dem", "rep", "yob", "pred.whi", "pred.bla", "pred.his", "median_income", "some_college")
...
saveRDS(df, "./temp/match_table_ma.rds")
