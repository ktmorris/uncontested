...
for(i in 1:1){
  fl_sample <- readRDS("./temp/florida_race_census.RDS") %>%
    ...
    sample_frac(0.01) %>%
    ungroup() %>% 
    dplyr::select(uncontested, dem, rep, yob, gender, white, black, latino, voted_primary, median_income, some_college)


  match_data <- fl_sample %>%
    dplyr::select(dem, rep, yob, gender, white, black, latino, voted_primary, median_income, some_college)
